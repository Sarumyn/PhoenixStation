/mob/living/carbon/alien/facehugger/emote(act,m_type=1,message = null)
...
//TRANSLATE EMOTE [V1.0]
	switch(act)
		...
		if("scratch")
			if (!src.restrained())
				message = "<B>[src.name]</B> издаёт звук скрежета."
				m_type = 1
		if("whimper")
			if (!muzzled)
...
//				m_type = 2
		...
		if("gasp")
			...
			m_type = 2
		if("shiver")
			message = "<B>[src.name]</B> испытывает озноб."
			m_type = 2
		if("drool")
		...
		if("scretch")
			if (!muzzled)
				message = "<B>[src.name]</B> издаёт шорох."
				m_type = 2
		if("choke")
...
//			m_type = 1
		...
		if("help")
			...
			to_chat(src, text("Неизвестна[LETTER_255] эмоци[LETTER_255]: []", act))
