	...
	switch(act)
		...
		if ("burp")
			if (!muzzled)
				...
				m_type = 2
		if ("deathgasp")
			message = "<B>[src]</B> издаёт противный визг огушающий вас, пузыр&#255;ща&#255;с&#255; зелёна&#255; кровь вытекает из ее пасти и вы видете как оно замирает..."
			m_type = 2
		if("scratch")
			if (!src.restrained())
		...
		if("roar")
			if (!muzzled)
				message = "<B>[src.name]</B> рычит."
				m_type = 2
		if("hiss")
			if(!muzzled)
				message = "<B>[src.name]</B> шипит."
				m_type = 2
		if("tail")
