/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M, mob/source)
...
//----------------------------------------------===HURT===----------------------------------------------//
		if("hurt")
			if(M.in_defense_mode)
				to_chat(M, "<span class='notice'>You cannot punch [src], because you are in defense.</span>")
				return 0
			M.do_attack_animation(src)
			receive_damage()
			...
			var/mult_damage = 0 //AWWARE UPDATE

			var/mult_evade = 0

			var/mult_weaken = 0

			var/dam = pick(BP_CHEST, BP_L_ARM, BP_R_ARM)

			var/random_buff_on_knockout = rand(5, 25)

			var/knock_random = rand(0, 12)

			var/damage = rand(0, 5)//BS12 EDIT

			var/evade_rand = abs((rand(0, 6) - mult_evade)) //AWWARE UPDATE

			var/weaken_rand = abs(( rand(0, 8) - mult_weaken))
			...
			switch(M.Weight)
				if(78 to 89) mult_damage = mult_damage + 1
				if(90 to 100)
					mult_damage = mult_damage + 1
					mult_weaken = mult_weaken + 1
				if(101 to 120)
					mult_damage = mult_damage + 2
					mult_weaken = mult_weaken + 2
			switch(M.Growth)
				if(176 to 210) mult_damage = mult_damage + 1
				if(155 to 165) mult_evade = mult_evade + 3
			...
			msg_admin_attack("[key_name(M)] [pick(attack.attack_verb)]ed [key_name(src)]")
																																						...
																																						//DEBUG
																																					//	visible_message("\red <B>TEST DEBUGING EVADE RAND : [evade_rand]</B>")
																																					//	visible_message("\blue <B>TEST DEBUGING WEAKEN RAND : [weaken_rand]</B>")
			...
			if(weaken_rand == 0) //Repaired weaken random.
				visible_message("\red <B>[M] has weakened [src]!</B>")
				apply_effect(2, WEAKEN, armor_block)

			damage += attack.damage
			debugdmg = (damage + mult_damage)

		//	visible_message("\red <B>WEIGH : [M.Weight]</B>") //get_weight_n()
		//	visible_message("\blue <B>TEST : [mult_damage]</B>")
//----------------------------------------------DEFENSE ACT----------------------------------------------//
			if(src.in_defense_mode) //AWWARE UPDATE
				BP = bodyparts_by_name[ran_zone(dam)]
				apply_damage(abs((debugdmg - 3)), BRUTE, BP, armor_block, attack.damage_flags())
//----------------------------------------------KNOCK----------------------------------------------//
			else if(!src.in_defense_mode && src.lying && !knocked && M.zone_sel.selecting == BP_HEAD && !istype(head, /obj/item/clothing/head/helmet))
				knocked = 1
				for (var/obj/item/weapon/grab/G in grabbed_by)
					if (G.state >= GRAB_AGGRESSIVE)
						grabbing = 1
				if(grabbing && knock_random == 0)
					visible_message("\red <B>[M] knocked [src]!</B>") //knock.ogg
					BP = bodyparts_by_name[ran_zone(BP_HEAD)]
					playsound(loc, 'sound/effects/knock.ogg', 50, 1, -1)
					apply_damage((debugdmg + random_buff_on_knockout), BRUTE, BP, armor_block, attack.damage_flags())
					apply_effect(55, PARALYZE, armor_block)
					adjustOxyLoss(70 * REAGENTS_EFFECT_MULTIPLIER)
					emote("knocked")
					source.stat = UNCONSCIOUS
					knocked = 0
				else
					apply_damage(debugdmg, BRUTE, BP, armor_block, attack.damage_flags())
//----------------------------------------------KNOCK----------------------------------------------//
			else
				apply_damage(debugdmg, BRUTE, BP, armor_block, attack.damage_flags()) //DEFAULT
//----------------------------------------------HURT----------------------------------------------//
...
/mob/living/carbon/human/proc/get_multiplay228(mob/living/carbon/human/M)
	var/temp = 0
	switch(M.Weight)
		if(90 to 120)
			temp = 12500
