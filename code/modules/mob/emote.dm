...
/mob/proc/custom_emote(m_type=1,message = null)
	...
	if(stat || !use_me && usr == src)
		to_chat(usr, "Вы отключили эмоции (me).")
		return
 ...
 // Maybe some people are okay with that.
		...
		for(var/mob/M in player_list)
			...
			if(istype(M, /mob/new_player))
				continue
			if(findtext(message," сопит.")) //Because we have so many sleeping people.
				break
...
/mob/proc/emote_dead(message)
	...
	if(client.prefs.muted & MUTE_DEADCHAT)
		to_chat(src, "\red Вы не можете показывать эмоции в : DEAD_CHAT (muted).")
		return

	if(!(client.prefs.chat_toggles & CHAT_DEAD))
		to_chat(src, "\red Вы не можете говорить в : DEAD_CHAT (muted).")
		return

	if(!src.client.holder)
		if(!dsay_allowed)
			to_chat(src, "\red DEAD CHAT - Globally muted")
			return
