/datum/preferences/proc/ShowGeneral(mob/user)
	...
	. += 						"<br>(<a href='?_src_=prefs;preference=name;task=random'>Случайная ФИ</a>)"
	. += 						"(<a href='?_src_=prefs;preference=name'>Всегда случайная ФИ: [be_random_name ? "Да" : "Нет"]</a>)"
	...
	. += 							"</td>"
	. += 						"</table>"
	. += 						"<b>Пол:</b> <a href='?_src_=prefs;preference=gender'><b>[gender == MALE ? "Мужской" : "Женский"]</b></a>"
	. += 						"<br><b>Возраст:</b> <a href='?_src_=prefs;preference=age;task=input'>[age]</a>"
	. += 						"<br><b>Выбор случайного слота:</b> <a href='?_src_=prefs;preference=randomslot'><b>[randomslot ? "Да" : "Нет"]</b></a>"
	. += 						"<hr>"
	. += 					"</td>"
	. += 				"</tr>"
	...
	. += 						"[submenu_type=="disabil_menu"?"<b>Изъяны</b>":"<a href=\"byond://?src=\ref[user];preference=disabil_menu\">Изъяны</a>"] - "
	...
	switch(submenu_type)	//Submenu
		...
		if("body")
			...
			. += "<br>Группа крови: <a href='byond://?src=\ref[user];preference=b_type;task=input'>[b_type]</a>"
			. += "<br>Цвет кожи: <a href='?_src_=prefs;preference=s_tone;task=input'>[-s_tone + 35]/220</a>"
			. += "<br>Рост: <a href='?_src_=prefs;preference=growth;task=input'>[Growth]cm</a>"
			. += "<br>Вес: <a href='?_src_=prefs;preference=weight;task=input'>[Weight]kg</a>"
			. += "<br>Телосложение: <a href='byond://?src=\ref[user];preference=body_type;task=input'>@[body_type]@</a>"

		//Organs
		if("organs")
			. += "Конечности и внутренние органы: <a href='byond://?src=\ref[user];preference=organs;task=input'>Установить</a>"
		...
		if("appearance")
			. += "<b>Волосы</b>"
			. += "<br><a href='?_src_=prefs;preference=hair;task=input'>Изменить цвет</a> <font face='fixedsys' size='3' color='#[num2hex(r_hair, 2)][num2hex(g_hair, 2)][num2hex(b_hair, 2)]'><table border cellspacing='0' style='display:inline;' bgcolor='#[num2hex(r_hair, 2)][num2hex(g_hair, 2)][num2hex(b_hair)]'><tr><td width='20' height='15'></td></tr></table></font>"
			. += " Style: <a href='?_src_=prefs;preference=h_style;task=input'>[h_style]</a><br>"
			. += "<b>Борода</b>"
			. += "<br><a href='?_src_=prefs;preference=facial;task=input'>Изменить цвет</a> <font face='fixedsys' size='3' color='#[num2hex(r_facial, 2)][num2hex(g_facial, 2)][num2hex(b_facial, 2)]'><table border cellspacing='0' style='display:inline;' bgcolor='#[num2hex(r_facial, 2)][num2hex(g_facial, 2)][num2hex(b_facial)]'><tr><td width='20' height='15'></td></tr></table></font>"
			. += " Стиль: <a href='?_src_=prefs;preference=f_style;task=input'>[f_style]</a><br>"
			. += "<b>Глаза</b>"
			. += "<br><a href='?_src_=prefs;preference=eyes;task=input'>Изменить цвет</a> <font face='fixedsys' size='3' color='#[num2hex(r_eyes, 2)][num2hex(g_eyes, 2)][num2hex(b_eyes, 2)]'><table border cellspacing='0' style='display:inline;' bgcolor='#[num2hex(r_eyes, 2)][num2hex(g_eyes, 2)][num2hex(b_eyes)]'><tr><td width='20' height='15'></td></tr></table></font><br>"
			. += "<b>Цвет тела</b>"
			. += "<br><a href='?_src_=prefs;preference=skin;task=input'>Изменить цвет</a> <font face='fixedsys' size='3' color='#[num2hex(r_skin, 2)][num2hex(g_skin, 2)][num2hex(b_skin, 2)]'><table border cellspacing='0' style='display:inline;' bgcolor='#[num2hex(r_skin, 2)][num2hex(g_skin, 2)][num2hex(b_skin)]'><tr><td width='20' height='15'></td></tr></table></font>"

		//Adjustment
		if("disabil_menu")
			. += "<b>Изъяны:</b>"
			. += "<br>"
			. += ShowDisabilityState(user,DISABILITY_NEARSIGHTED,"Нуждается в очках")
			. += ShowDisabilityState(user,DISABILITY_COUGHING,"Кашель")
			. += ShowDisabilityState(user,DISABILITY_EPILEPTIC,"Приступы")
			. += ShowDisabilityState(user,DISABILITY_TOURETTES,"Дрожь")
			. += ShowDisabilityState(user,DISABILITY_NERVOUS,"Нервозность")
			. += ShowDisabilityState(user,DISABILITY_FATNESS,"Лишний вес")

		//Gear
		if("gear")
			. += "<b>Одежда:</b><br>"
			if(gender == MALE)
				. += "Нижнее бельё: <a href ='?_src_=prefs;preference=underwear;task=input'>[underwear_m[underwear]]</a><br>"
			else
				. += "Нижнее бельё: <a href ='?_src_=prefs;preference=underwear;task=input'>[underwear_f[underwear]]</a><br>"
			. += "Нижняя рубаха: <a href='?_src_=prefs;preference=undershirt;task=input'>[undershirt_t[undershirt]]</a><br>"
			. += "Носки: <a href='?_src_=prefs;preference=socks;task=input'>[socks_t[socks]]</a><br>"
			. += "Тип сумки: <a href ='?_src_=prefs;preference=bag;task=input'>[backbaglist[backbag]]</a>"
	...
	. += 						"<br>Лояльность: <a href ='?_src_=prefs;preference=nt_relation;task=input'>[nanotrasen_relation]</a>"
	. += 						"<br>Система</b>: <a href='byond://?src=\ref[user];preference=home_system;task=input'>[home_system]</a>"
	. += 						"<br>Гражданство</b>: <a href='byond://?src=\ref[user];preference=citizenship;task=input'>[citizenship]</a>"
	...
	switch(href_list["preference"])
		...
		if("records")
			switch(href_list["task"])
				if("med_record")
					var/medmsg = sanitize(copytext(input(usr,"Set your medical notes here.","Medical Records",html_decode(revert_ja(med_record))) as message, 1, MAX_PAPER_MESSAGE_LEN), list("я"=LETTER_255))

					if(medmsg != null)
						med_record = medmsg

				if("sec_record")
					var/secmsg = sanitize(copytext(input(usr,"Set your security notes here.","Security Records",html_decode(revert_ja(sec_record))) as message, 1, MAX_PAPER_MESSAGE_LEN), list("я"=LETTER_255))

					if(secmsg != null)
						sec_record = secmsg

				if("gen_record")
					var/genmsg = sanitize(copytext(input(usr,"Set your employment notes here.","Employment Records",html_decode(revert_ja(gen_record))) as message, 1, MAX_PAPER_MESSAGE_LEN), list("я"=LETTER_255))
	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("body_type")
					var/new_body_type = input(user, "Choose your character's body type:", "Character Preference") as null|anything in list( "Стройное", "Полное", "Спортивное", "Плотное", "Вмеру упитанное", "Худощавое")
					if(new_body_type)
				...
				if("home_system")
					var/choice = input(user, "Please choose a home system.") as null|anything in home_system_choices + list("Нет","Другое")
					if(!choice)
						return
					if(choice == "Другое")
				...
				if("citizenship")
					var/choice = input(user, "Please choose your current citizenship.") as null|anything in citizenship_choices + list("Нет","Другое")
					if(!choice)
						return
					if(choice == "Другое")
				...
				if("faction")
					var/choice = input(user, "Please choose a faction to work for.") as null|anything in faction_choices + list("Нет","Другое")
					if(!choice)
						return
					if(choice == "Другое")
				...
				if("religion")
					var/choice = input(user, "Please choose a religion.") as null|anything in religion_choices + list("Нет","Другое")
					if(!choice)
						return
					if(choice == "Другое")
