...
/datum/geoip_data/proc/get_geoip_data(client/C, addr)
	...
	if(status == "updated")
		if(proxy == "true")
			...
			AddBan(C.ckey, C.computer_id, reason, "atom", 0, 0, C.mob.lastKnownIP)
			to_chat(C, "<span class='danger'><BIG><B>You have been banned by Phoenix.\nReason: [reason].</B></BIG></span>")
			...
			ban_unban_log_save("Phoenix has permabanned [C.ckey]. - Reason: [reason] - This is a permanent ban.")
			log_admin("Phoenix has banned [C.ckey].\nReason: [reason]\nThis is a permanent ban.")
			message_admins("Phoenix has banned [C.ckey].\nReason: [reason]\nThis is a permanent ban.")
