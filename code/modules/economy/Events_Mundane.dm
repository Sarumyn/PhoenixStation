...
/datum/event/mundane_news/announce()
	...
	newMsg.author = "Atom Station Daily"
		...
		switch(event_type)
			...
			if(GOSSIP)
				...
				if(prob(33))
					...
					"the decision to be dropped by a major sponsor","a disasterous interview on Atom Tonight")]."
			if(TOURISM)
				...
				Atom Station Daily is offering discount tickets for two to see [random_name(pick(MALE,FEMALE))] live in return for eyewitness reports and up to the minute coverage."

	for(var/datum/feed_channel/FC in news_network.network_channels)
		if(FC.channel_name == "Atom Station Daily")
			FC.messages += newMsg
			break
	for(var/obj/machinery/newscaster/NEWSCASTER in allCasters)
		NEWSCASTER.newsAlert("Atom Station Daily")
...
/datum/event/trivial_news/announce()
	...
	"[pick("Unathi","Spacer")] gets tattoo of Atom on chest '[pick("CentComm","star","starship","asteroid")] tickles most'.",\
	...
	"Survey: 'Cheese Louise' Voted Best Pizza Restaurant In Atom",\
